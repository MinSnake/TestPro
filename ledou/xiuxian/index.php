...
function oauth_authenticate()
{
    ...
    $start_time = ToolUtil::getMillisecond();
    $config = REQUEST_CONFIG['secure']['oauth/authenticate'];
    $host = TEST_HOST['secure'];
    $url = $host . '/' . $config['url'];
    $data = $config['data'];
...
function oauth_access_token()
{
    ...
    $start_time = ToolUtil::getMillisecond();
    $config = REQUEST_CONFIG['secure']['oauth/access_token'];
    $data = $config['data'];
...
function account_verify_credentials()
{
    ...
    $start_time = ToolUtil::getMillisecond();
    $config = REQUEST_CONFIG['feed']['account/verify_credentials'];
    $data = $config['data'];
    $method = $config['method'];
    $host = TEST_HOST['feed'];
    $url = $host . '/' . $config['url'];
    ...
    $base_string = $oauthUtil->base_string($method, $url, $head, array());
    $hash_string = $oauthUtil->hashsign($base_string, GAME_CONFIG['secret'], TOKEN_CONFIG['secret']);
    $base_string2 = $oauthUtil->base_string($method, $url, $head, $data);
