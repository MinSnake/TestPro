...
class FtpApiHelper{
	...
	public function __construct( $config = array() ){
		//加载配置
		$this->set_error( 0 );
		$this->config = $config ?: Yii::$app->params[ 'ftp_config' ];
		$this->enabled = false;
		if(empty( $this->config[ 'host' ] )) {
			$this->set_error( FTP_ERR_CONFIG_OFF );
			throw new NotAcceptableHttpException('获取上传FTP配置失败',FTP_ERR_CONFIG_OFF);
		} else {
			...
			if($this->func == 'ftp_connect' && ! function_exists( 'ftp_connect' )) {
				...
				throw new NotAcceptableHttpException('服务器不可用',FTP_ERR_SERVER_DISABLED);
			} else {
	 ...
	 * @throws NotAcceptableHttpException
	 */
	public function upload( $source, $extension = 'jpg', $target = '' ){
		if($this->error()) {
			return 0;
		}
		if(empty( $target )) {
			$target = date( 'Ym', time() ).'/'.uniqid().'.'.$extension;  //basename( $source );
		}
		...
		if( ! $this->ftp_chdir( $dirname )) {
			if($this->ftp_mkdir( $dirname )) {
				...
				if( ! $this->ftp_chdir( $dirname )) {
					...
					throw new NotAcceptableHttpException('FTP进入目录失败，请先检查是否有该目录'.$dirname,FTP_ERR_CHDIR);
				}
			} else {
				$this->set_error( FTP_ERR_MKDIR );
				throw new NotAcceptableHttpException('FTP新建文件夹失败，请先检查是否有'.$dirname.'目录的相关权限',FTP_ERR_MKDIR);
			}
		}
		$res = 0;
		//上传文件到目标目录
		if( ! $this->error()) {
			if($fp = @fopen( $source, 'rb' )) {
				...
				@fclose( $fp );
				if(!$res){
					$this->set_error( FTP_ERR_TARGET_WRITE );
					throw new NotAcceptableHttpException('FTP文件写入失败，请先检查是否有'.$dirname.'目录的相关权限',FTP_ERR_TARGET_WRITE);
				}
			} else {
				$this->set_error( FTP_ERR_SOURCE_READ );
				throw new NotAcceptableHttpException('FTP文件读取失败，请先检查是否有'.$dirname.'目录的相关权限',FTP_ERR_SOURCE_READ);
			}
		}
		$this->ftp_chdir( $old_dir );
		if($res) {
			$cdnLogEnable = Yii::$app->params['cdn_upload_log']; //配置文件
			if($cdnLogEnable){
				$content = $this->config[ 'attachdir' ].$target.PHP_EOL; //图片的路径
				$logFilePath = Yii::$app->getRuntimePath().'/logs/cdn_upload.log';//保存log 路径
				$LogResult  = file_put_contents($logFilePath, $content, FILE_APPEND); //写入
			}
			return $this->config[ 'attachdir' ].$target;
		} else {
			return 0;
		}
	}
	...
	function connect(){
		if( ! $this->enabled || empty( $this->config )) {
			...
			return $this->ftp_connect( $this->config[ 'host' ], $this->config[ 'username' ], $this->config[ 'password' ],
                $this->config[ 'attachdir' ], $this->config[ 'port' ], $this->config[ 'timeout' ], $this->config[ 'ssl' ],
                $this->config[ 'pasv' ] );
		}
		...
		if($this->connectid = $fun( $ftphost, $ftpport, 20 )) {
			...
			if($this->ftp_login( $username, $password )) {
				...
				if($this->ftp_chdir( $ftppath )) {
					...
					$this->set_error( FTP_ERR_CHDIR );
					throw new NotAcceptableHttpException('FTP进入目录失败，请先检查是否有'.$ftppath.'目录。如果有相关目录，可以将链接模式改成被动模式',FTP_ERR_CHDIR);
				}
			} else {
				$this->set_error( FTP_ERR_USER_NO_LOGGIN );
				throw new NotAcceptableHttpException('FTP用户登录失败，请先检查用户名和密码是否有误',FTP_ERR_USER_NO_LOGGIN);
			}
		} else {
			$this->set_error( FTP_ERR_CONNECT_TO_SERVER );
			throw new NotAcceptableHttpException('FTP连接失败，请先检查IP和端口是否有误',FTP_ERR_CONNECT_TO_SERVER);
		}
